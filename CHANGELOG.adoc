...
internal API.
* **[Archive]** Truncating a recording now stops any concurrent replays of that recording before proceeding with the deletion of segments.
* **[Event Logging]** The event debug logging when writing to a file allows for files to be rolled automatically based file size.
* **[Driver/C]** CPU affinity for driver threads is not CGroup aware. The user supplied CPU number for each thread can be an offset into the CPU set for the processes current CPU group.
* **[Driver/C]** CPU affinity is now supported for the background thread used for longer operations, e.g. name resolution.

=== Changelog

* **[Cluster]** Introduce `VersionValidator` interface and use it within the ConsensusModule and ClusteredService in place of the `AppVersionValidator` allowing for custom versions. `AppVersionValidator` remains the default.
